...
public class Degree implements AlgorithmUserFunction<Object, Object> {
    ...
    public void init(AlgorithmRuntimeContext<Object, Object> context, Object[] params) {
        ...
        if (params.length > 0) {
            throw new IllegalArgumentException("Only support no arguments, usage: degree()");
        }
    }

    @Override
    public void process(RowVertex vertex, Optional<Row> updatedValues, Iterator<Object> messages) {
        long inDegree = context.loadEdges(EdgeDirection.IN).size();
        long outDegree = context.loadEdges(EdgeDirection.OUT).size();
        context.take(ObjectRow.create(vertex.getId(), inDegree, outDegree));
    }
    ...
    public StructType getOutputType(GraphSchema graphSchema) {
        return new StructType(
            ...
            new TableField("in_degree", LongType.INSTANCE, false),
            new TableField("out_degree", LongType.INSTANCE, false)
        );
    }
}
