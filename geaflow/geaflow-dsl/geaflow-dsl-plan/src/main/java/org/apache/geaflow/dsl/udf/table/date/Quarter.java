...
@Description(name = "quarter", description = "Returns the quarter of the year for the given date.")
public class Quarter extends UDF {
    ...
    public Integer eval(String dateString) {
        ...
        try {
            int month = formatter.parseDateTime(dateString).getMonthOfYear();
            return (month - 1) / 3 + 1;
        } catch (Exception e) {
    ...
    public Integer eval(Timestamp t) {
        ...
        int month = dateTime.withMillis(t.getTime()).getMonthOfYear();
        return (month - 1) / 3 + 1;
    }
}
