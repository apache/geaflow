...
public class LmdbStoreBuilder implements IStoreBuilder {

    private static final StoreDesc STORE_DESC = new LmdbStoreDesc();

    public IBaseStore getStore(DataModel type, Configuration config) {
        switch (type) {
            case KV:
                return new KVLmdbStore();
            case STATIC_GRAPH:
                return new StaticGraphLmdbStore();
            case DYNAMIC_GRAPH:
                return new DynamicGraphLmdbStore();
            default:
    ...
    public static class LmdbStoreDesc implements StoreDesc {
