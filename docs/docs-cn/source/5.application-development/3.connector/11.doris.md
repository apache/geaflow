...
| 参数名 | 是否必须 | 描述 |
| ---- | ---- | ---- |
| geaflow.dsl.doris.fenodes | 写入必须 | Doris FE 的 HTTP 地址列表(host:httpPort)，逗号分隔，Sink 通过它做 Stream Load。 |
...
| geaflow.dsl.doris.sink.format | 否 | Stream Load 的负载格式，csv 或 json，默认 csv。 |
